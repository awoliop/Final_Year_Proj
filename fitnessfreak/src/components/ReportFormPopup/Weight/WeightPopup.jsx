...
const WeightPopup = ({ setShowWeightPopup }) => {
  const color = "#ffc20e";
  ...
  const getWeight = async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        toast.error("Error in getting the weight data");
      }
    } catch (error) {
      toast.error("Error in getting the weight data");
      console.log(error);
  ...
  const deleteWeight = async (item) => {
    try {
      fetch(process.env.NEXT_PUBLIC_BACKEND_API + "/weighttrack/deleteweightentry", {
        ...
        .then((data) => {
          if (data.ok) {
            toast.success("weight item deleted successfully");
            getStep();
          } else {
            toast.error("Error  in deleting the weight item");
          }
        });
    } catch (error) {
      toast.error("Error in deleting weight item");
      console.error(error);
    }
  };
