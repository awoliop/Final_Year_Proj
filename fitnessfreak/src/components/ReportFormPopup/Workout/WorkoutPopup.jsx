...
const WorkoutPopup = ({ setShowWorkoutPopup }) => {
  const color = "#ffc20e";
  const [date, setDate] = React.useState(() => dayjs() || null);
  const [time, setTime] = React.useState(() => dayjs() || null);

  const [workout, setWorkout] = useState({ date: "", exercise: "", durationInMinutes: "" });
  ...
  const getWorkout = async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        toast.error("Error in getting the workout data");
      }
    } catch (error) {
      toast.error("Error in getting the workout data");
      console.log(error);
      ...
      fetch(process.env.NEXT_PUBLIC_BACKEND_API + "/workouttrack/deleteworkoutentry", {
        ...
        .then((data) => {
          if (data.ok) {
            ...
            toast.error("Error  in deleting the workout item");
          }
        });
    } catch (error) {
      toast.error("Error in deleting workout item");
      console.error(error);
    }
  };
